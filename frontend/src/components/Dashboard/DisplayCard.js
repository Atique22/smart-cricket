...
function DisplayCard({ fileData, source }) {
  ...
  const handleSubmit = (event) => {
    ...
    setComments([
      ...comments,
      { timestamp: currentTime, text: newComment, status: frameStatus },
    ]);
    setNewComment("");
    console.log(
      "comment here is with time:" + comments + " time: " + currentTime
    );
    console.log(
      "frameStatus here is with time:" + frameStatus + " time: " + currentTime
    );
    console.log(
      "selectedFrame here is with time:" +
        selectedFrame +
        " time: " +
        currentTime
    );
  ...
  return (
    <>
      <Card className="m-4 p-1 col-10 position-absolute top-10 end-0">
        <CloseButton />
        <Card.Body>
          <div className="card-header">Title: {fileData?.fileName}</div>
          ...
          <Card.Text>file link : {source}</Card.Text>
          ...
          <ul>
            ...
            {comments
              ...
              .map(({ text, status, selectedFrame }, index) => (
                <li key={index}>
                  {" "}
                  time: {currentTime} Comment: {text} Frame: {selectedFrame}{" "}
                  Status:({status})
                </li>
              ))}
          </ul>
